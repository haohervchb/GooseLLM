...
// ======================================================================================
// FlashAttention-2 Decode Paged Forward for V100 (SM70)
//
// Partition-based single-token decode: splits long KV sequences into partitions,
// computes local softmax per partition, then merges via global softmax rescaling.
// Designed for latency-optimal single-token decode on V100 where Triton JIT
// overhead dominates.
// ======================================================================================
#include <cuda.h>
...
__global__ void flash_attention_decode_partition_kernel(
    ...
    // Resolve block table entries for this partition
    ...
    // Phase 1: compute per-token scores and local max
    ...
    // Phase 3: compute weighted V sum for this partition
...
__global__ void flash_attention_decode_reduce_kernel(
    ...
    // Load per-partition max and find global max
    float local_max = -1.0e20f;
    ...
    for (int i = threadIdx.x; i < num_partitions; i += blockDim.x) {
        const int64_t stats_index =
            static_cast<int64_t>(batch_idx) * stats_stride0 +
            static_cast<int64_t>(head_idx) * stats_stride1 + i;
        const float weight = exp_sums[stats_index] *
            __expf(max_shared[i] - global_max);
...
void launch_flash_attention_decode_paged(
    ...
    flash_attention_decode_partition_kernel<D, PARTITION_SIZE>
...
at::Tensor flash_attention_decode_paged(
    ...
    TORCH_CHECK(tmp_out.is_cuda() && max_logits.is_cuda() &&
                exp_sums.is_cuda(),
    ...
    TORCH_CHECK(max_logits.dtype() == torch::kFloat32,
                "max_logits must be fp32");
    TORCH_CHECK(exp_sums.dtype() == torch::kFloat32,
                "exp_sums must be fp32");
    ...
    TORCH_CHECK(tmp_out.dim() == 4,
                "tmp_out must have shape [B_cap, H, P, D]");
    TORCH_CHECK(max_logits.dim() == 3,
                "max_logits must have shape [B_cap, H, P]");
    TORCH_CHECK(exp_sums.dim() == 3,
                "exp_sums must have shape [B_cap, H, P]");
    TORCH_CHECK(q.stride(-1) == 1, "q last dim must be contiguous");
    TORCH_CHECK(k_cache.stride(-1) == 1,
                "k_cache last dim must be contiguous");
    TORCH_CHECK(v_cache.stride(-1) == 1,
                "v_cache last dim must be contiguous");
    TORCH_CHECK(tmp_out.stride(-1) == 1,
                "tmp_out last dim must be contiguous");
    ...
    TORCH_CHECK(head_dim == tmp_out.size(3),
                "tmp_out head_dim mismatch");
        ...
        switch (partition_size) {                                        \
            case 256:                                                    \
                launch_flash_attention_decode_paged<HDIM, 256>(          \
                    ...
                    tmp_out, max_logits, exp_sums, softmax_scale,        \
                    stream);                                             \
                break;                                                   \
            case 512:                                                    \
            ...
            default:                                                     \
                TORCH_CHECK(false,                                       \
                    "Unsupported partition_size: ", partition_size);     \
        }                                                                \
    } while (0)

    switch (head_dim) {
        case 64:  LAUNCH_BY_PARTITION(64);  break;
        case 80:  LAUNCH_BY_PARTITION(80);  break;
        case 96:  LAUNCH_BY_PARTITION(96);  break;
        case 112: LAUNCH_BY_PARTITION(112); break;
        case 128: LAUNCH_BY_PARTITION(128); break;
        case 256: LAUNCH_BY_PARTITION(256); break;
        default:
            TORCH_CHECK(false,
                "Unsupported head_dim for paged decode: ", head_dim);
    }
